        ...
        .then(response => {
            if (response.ok) {
                ...
                return response.json().then(body => {
                    alert('Logout failed: ' + body.message);
                    throw new Error('Logout failed: ' + body.message);
                });
            }
        })
        .catch(error => {
    ...
    fetch('/login/check') // 로그인 상태 확인 API 호출
        .then(response => {
            if (!response.ok) { // 요청이 실패하거나 로그인 상태가 아닌 경우
                return response.json().then(body => {
                    throw new Error('Not logged in or other error: ' + body.message);
                });
            }
...
function register(event) {
    ...
    fetch('/members', {
        ...
        .then(response => {
            if (!response.ok) {
                return response.json().then(body => {
                    alert('Signup request failed: ' + body.message);
                    throw new Error('Signup request failed: ' + body.message);
                });
            }
